...
const PROGRAM_BUILD_VERSION = "4.33.0";
...
function createStreetlifting16WMesocycle(startDateISO, cal = {}) {
  ...
  function toDay(label, sets, reps, vx, primaryPct, backoffPct, topPct, accessoryList, backoffStyle, phase, topSingleFirst, topConfig) {
    ...
    // v4.33.0 M20b: 8 min yläraaja-prehab + dippi-spesifi pec-tear-prehab.
    // ElMaraghy 2012: pec major -repeämät tapahtuvat eccentric-ylikuormassa
    // arm extended+abducted+ER kylmänä. Push-up eccentric (3s lasku) lisätty
    // pec-insertion-stretchin valmisteluun ennen raskasta dippiä.
    const warmupArr = phase === "peaking" ? TO_WARMUP_PEAKING : [
      // 1. Cardio-priming (60-90s)
      { name: "Hyppynaru / Cardio-ergo", desc: "60-90 s @ RPE 4 — lihaslämpö (McGowan 2015)" },
      // 2. T-spine + scap mobility — Cools 2014
      { name: "Cat-camel + T-spine ext + scap wall slides", desc: "6 + 8/puoli + 8 — T-spine + scap mobility (Cools 2014)" },
      // 3. Posterior capsule + pec minor
      { name: "Cross-body stretch + pec minor stretch", desc: "2×30 s + 30 s/puoli (Manske 2010, Wilk 2013)" },
      // 4. Side-lying ER (Reinold 2004)
      { name: "Side-lying ER", desc: "1×12-15 (band tai 2-4 kg KB) — RC aktivaatio (Reinold 2004)" },
      // 5. Prone Y / full can (Reinold 2007)
      { name: "Prone Y / Full can", desc: "1×10 hidas, 1-2 kg — supraspinatus (Reinold 2007)" },
      // 6. Serratus aktivaatio (Decker 1999)
      { name: "Serratus punch + wall slide", desc: "1×10 band — serratus anterior (Decker 1999, Cools 2014 dyskinesia → vammat)" },
      // 7. Pec-insertion-prehab DIPPI-spesifi (ElMaraghy 2012 pec-tear-mekanismi)
      { name: "Band fly + push-up eccentric (3s lasku)", desc: "1×10 + 1×8 BW — pec-insertion stretch + eccentric prehab (ElMaraghy 2012, dippi-spesifi)" },
      // 8. Liike-spesifit primerit
  ...
  function laDay(label, muLoad, muSets, muNote, muVx, fsWeek, finisher) {
    ...
    // v4.33.0 M20b: LA yläraaja-prehab + MU-spesifyys + alaraaja-elementti fsWeek:lle
    // (LA tekee sekä MU-skill/load + 2. kyykky → tarvitaan molempien prehab).
    return { dayOfWeek:6, dayType:"volume", label:label || "LA — Muscle-up + Kevyt",
      warmup: [
        // 1. Cardio-priming
        { name: "Hyppynaru / Cardio-ergo", desc: "60-90 s @ RPE 4 — lihaslämpö" },
        // 2. T-spine + scap mobility (Cools 2014)
        { name: "Cat-camel + T-spine ext + scap wall slides", desc: "6 + 8/puoli + 8 — T-spine + scap mobility" },
        // 3. MU-spesifi: false grip hang + scapular pull-up
        { name: "Scapular pull-up + False Grip hang", desc: "2×10 + 3×20 s — lapa-aktivaatio + ranteet MU:n false grip -tukeen" },
        // 4. RC + posterior capsule
        { name: "Side-lying ER + Cross-body stretch", desc: "1×12-15 + 2×30 s — RC aktivaatio + posterior capsule (Reinold 2004, Manske 2010)" },
        // 5. Tyler twist (distal biceps prehab MU-vetoon — Holshouser 2020 critical)
        { name: "Tyler twist + reverse Tyler", desc: "1×10-12 (Flexbar) — distal biceps + epicondylitis prehab (Tyler 2010)" },
        // 6. Alaraaja-elementit fsWeek:n takia
        ...(fsWeek ? [
          { name: "Hip 90/90 + Cossack squat", desc: "30 s/puoli + 5/puoli — lonkka-mobiliteetti 2. kyykkyä varten" },
          { name: "Banded clamshell + glute bridge", desc: "12/p + 8/p — glute aktivaatio (Boren 2011)" },
        ] : []),
        // 7. MU-neural-primer
        { name: "Räjähtävä leuka BW", desc: "3×3 — max-nopeus, MU-vedon RFD-primer" },
        // 8. Liikekohtaiset rampit
