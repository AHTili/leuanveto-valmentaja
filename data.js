...
const PROGRAM_BUILD_VERSION = "4.34.3";
...
function createStreetlifting16WMesocycle(startDateISO, cal = {}) {
  ...
  function toDay(label, sets, reps, vx, primaryPct, backoffPct, topPct, accessoryList, backoffStyle, phase, topSingleFirst, topConfig) {
    ...
    const warmupArr = phase === "peaking" ? TO_WARMUP_PEAKING : (isIntensityWeek ? [
      // INTENSITY-vaiheen lyhennetty versio (~6-8 min): pois loaded mobility + PAPE,
      // pidä aktivaatio + SA/LT priming + PNF + slow-ecc BW dippi.
      ...
      { name: "BW dippi slow eccentric", desc: "1×3, 5 s lasku, 1 s pause alapositiossa — capsule capacity priming + neural" },
      { name: "Warmup ramp (primary)", desc: "40% × 5 · 55% × 3 · 70% × 2 · 85% × 1 → workset" },
    ] : [
      // FOUNDATION + STRENGTH täysi versio (~8-12 min):
      ...
      // 8-9 min: SPESIFI PRIMING (capacity exposure, ei failure)
      { name: "BW dippi slow eccentric", desc: "1×3, 5 s lasku, 1 s pause alapositiossa — capsule capacity priming + neural readiness (Liu 2025, Vetter 2022 ekstsentrinen overload)" },
      // 9-11 min: SPESIFI RAMP-UP
      { name: "Warmup ramp (primary)", desc: "40% × 5 · 55% × 3 · 70% × 2 · 85% × 1 → workset" },
      // 11+ (optional): PAPE — vain jos timing menee oikein, muuten skip
  ...
  function laDay(label, muLoad, muSets, muNote, muVx, fsWeek, finisher) {
    ...
    // v4.34.3: HALF-KNEELING KB BOTTOMS-UP PRESS — anterior-stabilizer-accessory
    // (Track C deep research Liike 2). Sijoitettu LA-päivälle MU:n jälkeen,
    // koska MU on jo upper-body-priming-tila (low-fatigue, ei lisää kuormitusta
    // MA:n leuka-primaryyn 48 h:n päässä). Bottoms-up-asento PAKOTTAA subscap +
    // SA + RC -sekvenssin koordinoimaan — täydentää face pull + ulkokierto -volyymia
    // (jotka kohdistuvat vain posteriorisesti). Foundation 2×6/puoli 12-16 kg,
    // Strength 2×5/puoli 16-20 kg, Intensity 1×5/puoli 16 kg (maintenance), Peaking POIS.
    // Coaching consensus + Andersen 2014 + Kim 2025 — ei suoraa lisäpainodippi-spesifiä RCT:tä.
    const isPeakingLA = fsPhase === "peaking";
    if (!isPeakingLA) {
      let bupSets, bupReps, bupKg, bupNote;
      if (isSkill || fsPhase === "foundation" || fsPhase === null || fsPhase === undefined) {
        bupSets = 2; bupReps = 6; bupKg = 14;
        bupNote = "Foundation — pohja-ylöspäin KB rack-asennossa, työnnä yläspäin kontrolloidusti. 12-16 kg. Subscap + SA + RC-sekvenssi pakottaa anterior-stabilizers-koordinaatioon.";
      } else if (fsPhase === "strength") {
        bupSets = 2; bupReps = 5; bupKg = 18;
        bupNote = "Strength — 16-20 kg. Pohjat-ylöspäin, kontrolloitu työntö ylös. Pakottaa SSC + SA + RC -koordinaation.";
      } else {
        // intensity
        bupSets = 1; bupReps = 5; bupKg = 16;
        bupNote = "Intensity maintenance — 1×5/puoli 16 kg. Säilytä neuromuscular pattern, älä lisää volyymiä.";
      }
      slots.push({
        role: "accessory",
        ...
        sets: bupSets, reps: bupReps,
        targetVx: 4,  // anterior-stabilizer-priming, ei failure-zone
        suggestedLoadKg: bupKg,
        velocityStop: null,
        allowVelocityInput: false,
        note: `${bupNote} Tempo 2-1-1-1, jokaiselle puolelle ${bupSets}×${bupReps}. Lepo 60-90 s puolien välissä. (Track C deep research v4.34.3)`,
      });
    }
