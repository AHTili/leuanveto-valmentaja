...
const PROGRAM_BUILD_VERSION = "4.34.2";
...
function createStreetlifting16WMesocycle(startDateISO, cal = {}) {
  ...
  function toDay(label, sets, reps, vx, primaryPct, backoffPct, topPct, accessoryList, backoffStyle, phase, topSingleFirst, topConfig) {
    ...
    // v4.34.2: TO yläraaja-prehab refaktoroitu käyttäjäpalautteen perusteella —
    // aiempi "akateeminen" prehab (sitaatit Cools 2014 / Reinold 2004 / ElMaraghy 2012
    // jne.) ei pumpannut rintaa eikä lämmittänyt olkapäitä riittävästi → atleetin
    // olkapäät jäykistyivät dippien aikana, palautuminen kärsi seuraavien päivien
    // close-grip-penkki/overhead-press-treeneissä. Uusi prehab on bulletproof:
    // jokainen liike on konkreettinen, sets/reps on selkeät, ei sitaatteja, ja
    // fokus on rintaan + delt + RC pumppauksessa joka oikeasti valmistaa raskaaseen
    // dippiin (RC-jumpat → vinopenkki käsipainoilla → BW-dippi).
    const warmupArr = phase === "peaking" ? TO_WARMUP_PEAKING : [
      // 1. Yleislämmittely
      { name: "Hyppynaru / käsiergometri", desc: "60-90 s — verenkierto käyntiin" },
      // 2. RC-jumpat (3 liikkeen ketju, kevyt + nopea)
      { name: "Band pull-apart", desc: "1×15 — posterior delt + scap retraction" },
      { name: "Band ulkokierto (kiertäjäkalvosin)", desc: "1×12/puoli — kevyt band, RC-aktivaatio" },
      { name: "Face pull", desc: "1×15 — kevyt, kontrolloitu, ulkokierto yläasennossa" },
      // 3. Pec + delt -lämmitys käsipainoilla
      { name: "Vinopenkki käsipainoilla", desc: "2×10 — 8-12 kg, kevyt, täysi ROM (rinta pumppautuu lämpimäksi)" },
      // 4. Liikemallin herätys
      { name: "Bodyweight dippi", desc: "2×5-8 — liikemallin herätys ennen kuormaa" },
      // 5. Spesifinen ramp-up
      { name: "Warmup ramp (primary)", desc: "40% × 5 · 55% × 3 · 70% × 2 · 85% × 1 → workset" },
    ];
