...
| Seuraava askel | OSA 1 **PUSHATTU + PUHELINVERIFIOITU** (Akseli 2026-06-13: kapea penkki ~138 ei 82, hauiskääntö ~63 ei 36,7 — oppi 8 -portti läpäisty render-pinnalla; origin = `04fb8b2`, 8 committia). → OSA 2 (alla). |

## 7b. Session-tulos — OSA 2 (2026-06-13, OPUS 4.8 -sessio)

| Kenttä | Arvo |
| --- | --- |
| Commitit | `dd6c949` OSA 1 puhelinverifioitu → OSA 2 · `a2e4394` A7 Käsipainopenkki katalogiin + lukko + OBS-044-löydös · `b4f5c07` versio 4.52.41. Backup-ankkuri `backup-pre-h018-osa2-dd6c949`. |
| **A6-premissi FALSIFIOITUI (runtime, oppi 2)** | Handoffin A6 ("treenin aikana luotu custom-liike ei persistoidu") ei toistu: **Liikkeet ➕ persistoi moitteettomasti** (reproduktio: luo → reload → säilyi); workout-flow "+ Liike" tarjoaa vain olemassa olevia (ei luontipolkua); `seedPresets` seedaa vain tyhjään pankkiin (ei pyyhi). Todennäköinen OBS-041-juuri: Käsipainopenkki luotu eri instanssissa kuin puhelin-backup, EI persistenssi-bugi. **Akseli ratifioi: OSA 2 = A7-only (ei in-workout-luonti-featurea).** **EI datakatoa** (0 orphan-settiä; 2.6.–11.6. = tekemättömiä päiviä). |
| Toteutuneet kriteerit | **A7** ✓ Käsipainopenkki (flätti, horisontaalityöntö, tier 3) + howTo/cue; vino jo "Incline dumbbell press" → ei duplikoitu. **PROGRAM_BUILD_VERSION ei tarvita** (verifioitu end-to-end: `ensureNewPresetMovements` ajetaan joka init ehdoitta, surface'aa puuttuvat presetit nimi-dedupilla → Akselin pankki saa liikkeen seuraavalla avaamisella; APP_VERSION-bump riittää PWA:lle). **A8** ✓ näkyy Liikepankissa + workout-flow "+ Liike" tarjoaa (state.movements). |
| **Verifiointi (workflow, 4 adversariaali-linssiä: 2 PASS / 2 PASS_WITH_CONCERNS / 0 FAIL)** | **Korrektius+surfacing** ✓ (Käsipainopenkki ×1, surface'aa ×1; SIM delete→reload re-added inline). **Scope-aita PASS** — 5 tiedostoa, **engine.js 0 diff**, OSA 1 -kortti-fix + weekPlan + H-015/H-016 koskemattomat, pilot 64/64 kuorma-neutraali. **PROGRAM_BUILD_VERSION PASS** — väite todistettu (2 erillistä storea/mekanismia; ei version-gatea liikepankille). **Testilaatu PASS** — lukko aito kaksisuuntainen. Selaintestit **785/789** (+7 OSA2-lukkoa). |
| Jäännökset (nostettu, ei piiloteta) | **(1) OBS-044** (pre-existing, backlogissa): 2 eksaktia katalogi-duplikaattia (Hollow body hold, L-sit hold) → kaksoiskappale pankkiin. Verifier: **oma handoff, EI blokkaa, vakavuus MATALA** (kuormaamattomat core-pidot, ei e1RM-/kuorma-vaikutusta); korjaus vaatii migraatio-heal'in (nimi-dedup ei heal'aa jo-asennettuja). OSA 2:n oma integriteetti puhdas (Käsipainopenkki ×1), T3-lukko vartioi. **(2) Custom-nimi-reuna:** jos käyttäjällä on EKSAKTI custom-"Käsipainopenkki", preset ei korvaa sitä (jää ilman metadataa) — ei koske Akselia (12.6.-backupissa ei ole Käsipainopenkkia → preset lisätään puhtaasti). |
| **A8 PUHELINVERIFIOINTI** | ✅ **KUITATTU (Akseli 2026-06-13, 4.52.41):** "Käsipainopenkki" löytyy sekä Liikepankista ETTÄ treenin "+ Liike" -valikosta. Render-pinta-acceptance täytetty (oppi 8 -portti). |
| Seuraava askel | **H-018 VALMIS — kokonaan suljettu + arkistoitu** (OSA 1 + OSA 2, origin = HEAD `<arkistointi-commit>`). Jäännökset OBS-042/043/044 backlogissa (omat pikahandoffit). H-016 dormantti, odottaa vk 25 live-porttia (ei arkistoida vielä). Seuraava engine-pala: D1 (intra-session-autoregulaatio v1) — tulee ratifioituna Cowork-handoffina, EI aloiteta autonomisesti. |
