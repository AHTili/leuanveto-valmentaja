...
### OBS-051 · 2026-06-17 · Akselin app-data (squat) · KORJATTU (A2, APP_VERSION 4.52.44)
...
### OBS-052 · 2026-06-18 · Akselin pistaritesti (squat 4×3 V2 → 150 vs 160) · KORJATTU (A2 kaksikerroksinen, APP_VERSION 4.52.45)
**Cal-priority-ikkuna kukistuu cal-ensin-järjestyksestä → luotettavin signaali (cal-testi ±2,7 kg) alikäytetty → Vx-epäjohdonmukainen kuorma.** Akselin periaate (ratifioitu): *"luotettavin signaali (cal/velocity/parhaat primary-setit) ohjaa e1RM:ää AINA — ei hauraasta mediaani/sarjajärjestys-mekanismista."* Juuri: cal-priority [engine.js ~4312] tarkisti `recentTopSets.slice(-3)` (3 viim. SARJAA) → cal tehdään treenin alussa → ≥3 työsarjaa jälkeen työntää cal:in ulos → deload-mediaani (172,7) voittaa, cal 187 jää lattiaksi ×0,95. Systeeminen: squat −8 %, leuka −7 % (liike-agnostinen). Vastakkainen suunta kuin OBS-048/051 (= ALI-käyttö). **A2-KORJAUS (kaksikerroksinen, OBS-048/051-perhe, ei OBS-029/velocity-raaka):**
- **KERROS 1 — resolveri (M1/K2b):** jaettu **order-immuuni** helper `mostRecentSessionCalibSets(sets)` (engine.js ~298): cal ajaa jos VIIM. SESSIOSSA (max-timestamp-sessio, EI taulukon häntä — kortti saa lajittelemattoman state.allSets:in). Sovellettu **3 LIVE-e1RM-source-sijaintiin:** recommend()-cal-priority + `computeMovementE1RMBest`-kortti + **`recommendPeaking()`** (3. sijainti, adversariaali löysi). → squat e1RM 172,7→**187**, primary 4×3@V2 147,5→**160,5** (= vReps(5), Vx-johdonmukainen).
- **KERROS 2 — break-override:** `computeMovementReload` [~1623] TILA 2: tuore cal paluujaksolla (timestamp > tauon-edeltävä sessio) = re-entry-testi → `return null` (ohittaa break-rampin). Akseli kalibroi 06-16-paluusessiossa (15-pv-tauon jälkeen) → ramppi kohti vanhaa 155:tä oli "sekoittava"; cal 187 ajaa. Kattaa cal-only-paluusession (aikaleima-vertailu, ei sessionId).
- **REUNAEHTO A (kriittinen, adversariaali-todennettu):** EI avaa velocity-RAAKAA prescriptioon — vain `setRole==="calibration"` ohittaa; KERROS 2 palauttaa vain null/medianLoad; OBS-029-suppressio (divergoiva velocity diff>5%) koskematon. *HUOM: cal-ankkurointi VOI siirtää KONVERGOIVAN velocity-estimaatin (≤5% cal:ista, clampattu cal×1,05) VBT-promote-portin sisään — ei raakavuoto vaan konvergenssi; uusi käyttäytyminen, jätetty pilari 1 -seurantaan.*
**Adversariaalinen verifiointi (4 kriitikkoa) löysi 3 BLOKKAAJAA jotka korjattu:** (1) helper order-dependence (taulukon häntä vs max-timestamp) → F-3 + H018-T2 rikko → order-immuuni; (2) `recommendPeaking()` ei migroitu (3. sijainti, kuten OBS-051:n 4 PLAN_BASED) → migroitu; (3) väärä "pilot bittitarkka" -väite → rehellinen LOAD-DIFF-SWEEP. Testilukot: `testE1rmCardCalDriverOrderIndep` (cal-driver + order-immuuni, lukitsee H018-T2) + `testReloadCalOverride` (KERROS 2 known-pos/neg). **LOAD-DIFF-SWEEP:** pilot 64/64, 0 virhettä, audit-flagit IDENTTISET (ei uusia patologioita), MUTTA EI bittitarkka — 34 cal-ankkuroitua kuormamuutosta (sallittu: cal nyt ajaa sessio-aware). Routing: M1/K2b (resolveri) + M2/pilari 1 (velocity-konvergenssi-seuranta). Liittyy: OBS-048/051 (e1RM-resolveri-perhe, ali-käyttö-suunta), OBS-050 (de-litistys). **KAPSTONI pilari 1 — ensimmäinen rakenteellinen toteutus (cal aidosti ohjaava).**
