...
### Intensiteetti-oikeellisuus-luokka SULJETTU (sweep, 2026-06-17)
Systeeminen sweep (kaikki slotit × 16 vk × 3 primary-liikettä, tier-rikastettu) vahvisti: kuorman-johdon valmennuksellisen oikeellisuuden luokka = **TÄSMÄLLEEN OBS-048 ∪ OBS-049 — ei kolmatta rikkomustyyppiä.** Molemmat korjattu rakenteellisesti resolver-tasolla (kattaa minkä tahansa skeletonin cal/secondary-slotit). Analogia: value-resolution sulki fragmentaation (F-1…F-5). Sweep-scope: streetlifting_16w + normaali historia; resolver-tason korjaus → rakenteellinen kattavuus.
