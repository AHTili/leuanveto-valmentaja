...
**PLAN_BASED-e1RM-inflaatio: live-kuorman squat-session-e1RM 196,3 (capattu 241,4:stä), yli terveen ~172–187.** `PLAN_BASED_E1RM` (engine.js:~4434) laskee `plan_e1rm = lastLoad / lastLoadPct`. vk8-primary 3×3@V4 tehtynä 140 kg @ ohjelmoitu loadPct **0,58** → 140/0,58 = **241,4** → INFLATION_CAP 187×1,05 = 196,3. **Juuri = loadPct (0,58) ≠ Vx-intensiteetti (vReps(7)=0,81):** PLAN_BASED luottaa volyymi-label-loadPct:hen tosi-%1RM:nä → jakaa väärällä %:lla → inflatoituu. Akselin oma diagnoosi: "kone luuli että sain lisävoimia kun nostin sarjapainoa — % ja vara eivät täsmänneet" = **kategoriavirhe** (suunnittelu-label kohdeltu voiman mittauksena). **OBS-048-sisarvika:** cal-base reititettiin kanoniseen (OBS-048), mutta työsarjapolku (currentE1RMSystem) jäi PLAN_BASED-inflatoituneeksi (F-3: kortti=Best, live-kuorma=currentE1RMSystem). **EI OBS-029/velocity** — VBT-cross-check (224,2) ei-promotoitu (diff 12,7 % > 5 %), red herring suljettu pois. **A2-KORJAUS:** `PLAN_BASED_VX_TOL = 0,15` + yksisuuntainen consistency-gate (engine.js:~4430): PLAN_BASED laukeaa vain kun `loadPct >= vReps(reps+Vx) × 0,85`; alittava (volyymi-label) → skippaa → Epley-Vara säilyy (→ DEFLATION-floor PR 185×0,95 = 175,75). Deflatoiva loadPct > vReps saa edetä. **Routing: K2b/e1RM-computation, ei M2/VBT.** Known-pos (app-data): gataa squat loadPct 0,55/0,58 (60 laukeamaa), säilyttää 0,71/0,78 + leuka/dippi (kaikki deflatoivia, gatattavia ei-squat-laukeamia = 0). Testilukko: PLAN_BASED_VX_GATED-scenario (S10 = consistent known-negative). Pilot 64/64 bittitarkka (0 diffiä). Liittyy: OBS-050 (oire), OBS-048 (sisarvika), OBS-029 (suljettu pois).
